...
后缀，也不得先签发 token 再让用户处理冲突。此时提供三个出口：修改为唯一名称后
继续授权、取消本次授权，或者“替换授权”。
...
- 同名错误态提供“替换授权”；点击后先展示旧连接摘要，再使用现有危险操作 modal
  二次确认；
...
7. 用户可以二次确认替换授权；新 token 交换失败时旧连接仍有效；
8. 并发提交同名连接时最多一个成功，另一个返回可恢复的名称冲突；
9. Codex 连接按客户端类型折叠，展开后名称互不重复且可逐个撤销；
10. “撤销全部”只撤销当前账号、MCP audience 下该分组的连接；
11. Runtime OAuth 不出现在 MCP OAuth 列表，设备页能显示可靠的设备名；
12. 历史重复项不丢失、不被静默撤销，并获得确定且唯一的迁移名称；
13. 数据库和日志中不存在硬件序列号、MAC、原始 iLink 标识或 OAuth token 原文。
