...
后缀，也不得先签发 token 再让用户处理冲突。页面始终只显示两个操作按钮：名称
唯一时为“继续”和“取消授权”；名称重复时为“继续并替换”和“取消授权”。修改名称
不是第三个操作，而是在同一个输入框中完成。

“替换授权”表示当前授权成功后，原来占用这个名称的连接自动失效。操作前必须展示
旧连接的客户端、授权时间和最近使用时间并二次确认。取消本次授权不得修改任何已有
连接；当前授权或 token exchange 失败时，旧连接也必须继续有效。

授权页首次展示默认名称时立即校验。用户编辑过名称后进行短延迟的异步重新校验；
校验进行中禁用主按钮并显示现有轻量加载状态。校验为唯一后清除同名错误和旧连接
摘要，主按钮显示“继续”；校验为重复时显示对应旧连接摘要，主按钮显示“继续并
替换”。前端校验只改善体验，服务端在提交和 token exchange 时都必须重新校验。

## 5. 授权与重新授权

1. DCR 创建 public client，并保存可选的受限设备元数据；
2. 授权页展示客户端、resource、scope 和连接名称；
3. 用户确认后，服务端校验连接名称唯一性。普通授权把待创建的 label 写入
   authorization code；替换授权还写入经过账号、audience 和名称校验的
   `replacement_connection_id`；
4. code exchange 在同一事务中创建或复用 connection、签发新 token，并在成功签发
   后撤销被替换 connection 的全部旧 token；
5. Runtime 稳定 installation 的重新授权在新 token 成功后撤销旧 token；
6. 普通 MCP 的重新授权需要用户提供唯一名称，或由用户明确选择替换授权；除非未来
   宿主提供稳定 instance identity，否则不会静默替代既有连接。

授权失败或用户取消时，不创建连接，也不撤销旧 token。替换授权必须以新 token
已经成功签发为边界，避免把用户锁在账号之外。

替换授权的 token exchange 必须锁定待替换 connection，并重新验证它仍属于
authorization code 的账号、audience 和 normalized label。撤销旧连接、创建新连接、
签发新 token 和消费 authorization code 必须属于同一个数据库事务；任一步失败都
整体回滚，旧连接继续有效。
...
- 替换授权只能替换占用当前 normalized label 的历史 connection，不能借该入口选择
  其他名称、账号或 audience 的连接；
...
- 授权页始终只有一个主按钮和一个“取消授权”按钮，不增加第三个并列操作；
- 名称唯一时主按钮为“继续”，名称重复时主按钮为“继续并替换”；
- 用户编辑名称后自动重新校验；校验中主按钮禁用，校验结果变化时按钮文案和旧连接
  摘要同步切换；
- 点击“继续并替换”后先使用现有危险操作 modal 展示旧连接摘要并二次确认；
- “取消授权”沿用现有 OAuth cancel 回调和页面样式；
...
6. 用户编辑为唯一名称后主按钮从“继续并替换”恢复为“继续”，且页面始终只有两个
   操作按钮；
7. 同名错误态可以取消授权，取消后客户端收到标准拒绝结果且已有连接不变；
8. 用户可以二次确认替换授权；新 token 交换失败时旧连接仍有效；
9. 并发提交同名连接时最多一个成功，另一个返回可恢复的名称冲突；
10. Codex 连接按客户端类型折叠，展开后名称互不重复且可逐个撤销；
11. “撤销全部”只撤销当前账号、MCP audience 下该分组的连接；
12. Runtime OAuth 不出现在 MCP OAuth 列表，设备页能显示可靠的设备名；
13. 历史重复项不丢失、不被静默撤销，并获得确定且唯一的迁移名称；
14. 数据库和日志中不存在硬件序列号、MAC、原始 iLink 标识或 OAuth token 原文。
